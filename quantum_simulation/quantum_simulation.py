...
alphas = [4/8*np.pi]                                        # list of hamiltonian parameter
...
for alpha in alphas:
    ...
    for phi0 in philist2:
        for theta0 in thetalist2:
            ...
            jobs.append(backend.run(transpile(circuits, backend), shots = shots))
            ...
            # Saving to txt file
            if savetotxt:
                np.savetxt(f'sym_{np.round(alpha,2)}_{np.round(theta0,2)}x{np.round(phi0,2)}_{N_H}.txt', resultlist)
            ...
            if plot:
                ...
                plt.tight_layout()
                plt.savefig(f'sym_{np.round(alpha,2)}_{np.round(theta0,2)}x{np.round(phi0,2)}_{N_H}.png')
                plt.clf()
